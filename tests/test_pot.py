...
class TestPot(BaseTest):
    ...
    def test_one(self, mock_one, mock_db):
        ...
        m = Pot.one("select * from pot where id = %s", [123])

        self.assertIsInstance(m, Pot)
        self.assertEqual(m.attributes, {'key': 'one', 'key2': 'two'})
        self.assertEqual(m.table, 'pot')
        self.assertEqual(m.factory_query, False)
    ...
    def test_find(self, mock_query, mock_init):
        ...
        resp = m.find('select * from pot where id = %s', [123])

        self.assertIsInstance(resp, list)
        self.assertEqual(len(resp), 1)
        self.assertIsInstance(resp[0], Pot)
    ...
    def test_related(self, mock_query, mock_init):
        ...
        m.related('transaction', 'pot_id', 123, 'id', 'asc', None, 0)

        mock_query.assert_called_with('select * from `transaction` where pot_id = %s and deleted = %s order by id asc', [123, 0])
    ...
    def test_save_create_new(self, mock_update, mock_create, mock_init):
        ...
        mock_create.assert_called_with('pot', {'key1': 'blah', 'key2': 'bloo'})
    ...
    def test_save_create_new(self, mock_update, mock_create, mock_init):
        ...
        mock_update.assert_called_with('pot', 123, {'id': 123, 'key1': 'blah', 'key2': 'bloo'})
    ...
    def test_delete_with_id(self, mock_query, mock_init):
        ...
        mock_query.assert_called_with('delete from pot where id = %s', [123])
    ...
    def test_factory(self, mock_find, mock_init):
        ...
        mock_find.assert_called_with('pot')
    ...
    @patch('monzo_utils.model.pot.Pot.factory')
    ...
    def test_join_no_relationships_defined(self):
        ...
        self.assertIn("AttributeError(\"'Pot' object has no attribute 'RELATIONSHIPS'\")", str(e))
    ...
    def test_join(self, mock_factory, mock_join, mock_init):
        ...
        mock_join.assert_called_with('pot', 'join column1', 'join column2')
    ...
    def test_left_join(self, mock_factory, mock_join, mock_init):
        ...
        resp = mp.leftJoin('pot', 'blah = 1')

        mock_factory.assert_called()

        mock_join.assert_called_with('pot', 'join column1', 'join column2', 'blah = 1')
