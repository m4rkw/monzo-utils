...
class TestProvider(BaseTest):
    ...
    def test_find(self, mock_query, mock_init):
        ...
        resp = m.find('select * from provider where id = %s', [123])

        self.assertIsInstance(resp, list)
        self.assertEqual(len(resp), 1)
        self.assertIsInstance(resp[0], Provider)
    ...
    def test_related(self, mock_query, mock_init):
        ...
        m.related('transaction', 'provider_id', 123, 'id', 'asc', None, 0)

        mock_query.assert_called_with('select * from `transaction` where provider_id = %s and deleted = %s order by id asc', [123, 0])
    ...
    def test_save_create_new(self, mock_update, mock_create, mock_init):
        ...
        mock_create.assert_called_with('provider', {'key1': 'blah', 'key2': 'bloo'})
    ...
    def test_save_create_new(self, mock_update, mock_create, mock_init):
        ...
        mock_update.assert_called_with('provider', 123, {'id': 123, 'key1': 'blah', 'key2': 'bloo'})
    ...
    def test_delete_with_id(self, mock_query, mock_init):
        ...
        mock_query.assert_called_with('delete from provider where id = %s', [123])
    ...
    def test_factory(self, mock_find, mock_init):
        ...
        mock_find.assert_called_with('provider')
    ...
    @patch('monzo_utils.model.provider.Provider.factory')
    ...
    def test_join_no_relationships_defined(self):
        ...
        self.assertIn("AttributeError(\"'Provider' object has no attribute 'RELATIONSHIPS'\")", str(e))
    ...
    def test_join(self, mock_factory, mock_join, mock_init):
        ...
        mp = Provider()
        mp.RELATIONSHIPS = {
            'provider': ['join column1', 'join column2']
        }

        resp = mp.join('provider')

        mock_factory.assert_called()

        mock_join.assert_called_with('provider', 'join column1', 'join column2')
    ...
    def test_left_join(self, mock_factory, mock_join, mock_init):
        ...
        resp = mp.leftJoin('provider', 'blah = 1')

        mock_factory.assert_called()

        mock_join.assert_called_with('provider', 'join column1', 'join column2', 'blah = 1')
    ...
    def test_getone(self, mock_related):
        account1 = Account()
        account1.attributes = {'name': 'Test1'}
        account2 = Account()
        account2.attributes = {'name': 'Test2'}

        mock_related.return_value = [account1, account2]
        ...
        self.assertIsInstance(resp[1], Account)
        self.assertEqual(resp[0].name, 'Test1')
        self.assertEqual(resp[1].name, 'Test2')
