...
class TestCounterparty(BaseTest):
    ...
    def test_constructor(self):
        ...
        self.assertEqual(m.table, 'counterparty')
    ...
    def test_one(self, mock_one, mock_db):
        ...
        m = Counterparty.one("select * from counterparty where id = %s", [123])

        self.assertIsInstance(m, Counterparty)
        self.assertEqual(m.attributes, {'key': 'one', 'key2': 'two'})
        self.assertEqual(m.table, 'counterparty')
        self.assertEqual(m.factory_query, False)
    ...
    def test_find(self, mock_query, mock_init):
        ...
        resp = Counterparty.find('select * from counterparty where id = %s', [123])

        self.assertIsInstance(resp, list)
        self.assertEqual(len(resp), 1)
        self.assertIsInstance(resp[0], Counterparty)
    ...
    def test_related(self, mock_query, mock_init):
        ...
        m.related('transaction', 'counterparty_id', 123, 'id', 'asc', None, 0)

        mock_query.assert_called_with('select * from `transaction` where counterparty_id = %s and deleted = %s order by id asc', [123, 0])
    ...
    def test_save_create_new(self, mock_update, mock_create, mock_init):
        ...
        mock_create.assert_called_with('counterparty', {'key1': 'blah', 'key2': 'bloo'})
    ...
    def test_save_create_new(self, mock_update, mock_create, mock_init):
        ...
        mock_update.assert_called_with('counterparty', 123, {'id': 123, 'key1': 'blah', 'key2': 'bloo'})
    ...
    def test_delete_with_id(self, mock_query, mock_init):
        ...
        mock_query.assert_called_with('delete from counterparty where id = %s', [123])
    ...
    def test_factory(self, mock_find, mock_init):
        ...
        mp.table = 'counterparty'

        mp.factory()

        self.assertEqual(mp.factory_query, True)

        mock_find.assert_called_with('counterparty')
    ...
    @patch('monzo_utils.model.counterparty.Counterparty.factory')
    ...
    def test_join_no_relationships_defined(self):
        ...
        with pytest.raises(Exception) as e:
            mp.join('counterparty')

        self.assertIn("AttributeError(\"'Counterparty' object has no attribute 'RELATIONSHIPS'\")", str(e))
    ...
    def test_join(self, mock_factory, mock_join, mock_init):
        ...
        mp = Counterparty()
        mp.RELATIONSHIPS = {
            'counterparty': ['join column1', 'join column2']
        }

        resp = mp.join('counterparty')

        mock_factory.assert_called()

        mock_join.assert_called_with('counterparty', 'join column1', 'join column2')
    ...
    def test_left_join_no_relationships_defined(self):
        ...
        with pytest.raises(Exception) as e:
            mp.leftJoin('counterparty', 'blah = 1')
    ...
    def test_left_join(self, mock_factory, mock_join, mock_init):
        ...
        resp = mp.leftJoin('counterparty', 'blah = 1')

        mock_factory.assert_called()

        mock_join.assert_called_with('counterparty', 'join column1', 'join column2', 'blah = 1')
