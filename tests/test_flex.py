...
class TestFlex(BaseTest):
    ...
    def test_data_fields(self, mock_query, mock_db):
        ...
        self.assertEqual(data['amount'], 41)
        ...
        self.assertEqual(data['payment_type'], 'Flex')
        self.assertEqual(data['remaining'], 82)
        self.assertEqual(data['status'], 'PAID')
        self.assertEqual(data['suffix'], '1/3')
    ...
    def test_display(self, mock_print, mock_query, mock_db):
        ...
        mock_print.assert_called_with('   PAID: Flex            - payment                 1/3   £41.00              £82.00   2024-01-01   2024-02-01')
    ...
    @patch('monzo_utils.lib.db.DB.__init__')
    @patch('monzo_utils.lib.db.DB.query')
    @patch('monzo_utils.model.flex.Flex.due_date', new_callable=PropertyMock)
    ...
    def test_payment_type(self, mock_query, mock_db):
        ...
        self.assertEqual(p.payment_type, 'Flex')
    ...
    def test_display_amount_override(self, mock_query, mock_db):
        ...
        self.assertEqual(p.display_amount, 41)


    @patch('monzo_utils.lib.db.DB.__init__')
    @patch('monzo_utils.lib.db.DB.query')
    @patch('monzo_utils.model.flex.Flex.last_payment', new_callable=PropertyMock)
    ...
    def test_next_month_amount(self, mock_query, mock_db):
        ...
        with freeze_time("2024-01-20"):
            self.assertEqual(p.next_month_amount, 41)
    ...
    @patch('monzo_utils.lib.db.DB.query')
    @patch('monzo_utils.model.flex.Flex.last_payment', new_callable=PropertyMock)
    @patch('monzo_utils.model.flex.Flex.older_last_payment', new_callable=PropertyMock)
    ...
    def test_get_transaction_where_condition__desc_list(self, mock_query, mock_db):
        ...
        with freeze_time("2024-01-20"):
            ...
            self.assertEqual(where, 'account_id = %s and money_out > %s and declined = %s and (  description like %s  or  description like %s ) and `date` >= %s')
            self.assertEqual(params, [1, 0, 0, '%desc1%', '%desc2%', '2024-01-01'])
    ...
    @patch('monzo_utils.model.flex.Flex.get_transaction_where_condition')
